...
function buildFinancialReport({ scopedData = {}, insight = {}, generatedAt = new Date() } = {}) {
  ...
  const summary = [
    ...
    ['Debt repayment for next budget', formatVnd(insight.debt_repayment_amount || 0)],
    ['Surplus available for next period', formatVnd(insight.surplus_to_carry_next_period || 0)],
    ['Budget carried to next period', formatVnd(insight.budget_carry_to_next_period || 0)],
    ['Saving goal contribution', formatVnd(insight.saving_goal_contribution_amount || 0)],
    ['Next budget preview', formatVnd(insight.next_budget_preview_amount || 0)],
...
function toExcelHtml(report) {
  ...
  const debtRows = report.debtHistory
    .map((row) => `<tr><td>${escapeHtml(row.periodLabel)}</td><td>${row.grossBudget}</td><td>${row.carriedDebtFromPrevious}</td><td>${row.availableBudget}</td><td>${row.periodExpense}</td><td>${row.surplusAmount || 0}</td><td>${row.budgetCarryToNextPeriod || 0}</td><td>${row.savingGoalContributionAmount || 0}</td><td>${row.debtToCarryNextPeriod}</td></tr>`)
    .join('');
...
<table><tr><th>Period</th><th>Gross budget</th><th>Debt from previous</th><th>Available budget</th><th>Expense</th><th>Surplus</th><th>Budget carry</th><th>Saving contribution</th><th>Debt to next</th></tr>${debtRows}</table>
<h2>Transactions</h2>
...
function toPdfBuffer(report) {
  ...
  report.debtHistory.slice(-6).forEach((row) => {
    lines.push(`${row.periodLabel}: gross ${formatVnd(row.grossBudget)}, previous debt ${formatVnd(row.carriedDebtFromPrevious)}, available ${formatVnd(row.availableBudget)}, expense ${formatVnd(row.periodExpense)}, surplus ${formatVnd(row.surplusAmount || 0)}, budget carry ${formatVnd(row.budgetCarryToNextPeriod || 0)}, saving ${formatVnd(row.savingGoalContributionAmount || 0)}, next debt ${formatVnd(row.debtToCarryNextPeriod)}`);
  });
