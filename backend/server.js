...
const { applyDebtCarryoverDecision } = require('./src/modules/finance/debtCarryoverDecision');
const {
...
} = require('./src/repositories/debtCarryoverRepository');
const {
...
function mergeDebtCarryoverRecords(primaryRecords = [], secondaryRecords = []) {
  const byPeriod = new Map();
  [...secondaryRecords, ...primaryRecords].forEach((record) => {
    if (!record?.periodKey) return;
    byPeriod.set(record.periodKey, record);
  });
  return [...byPeriod.values()].sort((a, b) => new Date(b.updatedAt || b.createdAt || 0) - new Date(a.updatedAt || a.createdAt || 0));
}

function buildUserScopedData(data, user) {
  const userId = getUserId(user);
  const jsonDebtRecords = (data.debtCarryovers || []).filter((item) => hasUserOwner(item, user));
  const separateDebtRecords = userId ? getUserDebtCarryoverRecordsSync(userId) : [];
  return {
    ...data,
    ...
    debtCarryovers: mergeDebtCarryoverRecords(jsonDebtRecords, separateDebtRecords)
  };
}

function buildDebtSnapshotRecordFromInsight(user, insight = {}) {
  return {
    userId: getUserId(user),
    periodKey: insight.debt_period_key || new Date().toISOString().slice(0, 7),
    periodLabel: insight.debt_period_label || 'Kỳ hiện tại',
    periodType: insight.debt_period_type || 'month',
    strategy: insight.debt_to_carry_next_period > 0 ? 'auto_overspending_snapshot' : 'auto_carryover_snapshot',
    baseBudgetAmount: insight.base_budget_amount || 0,
    periodIncome: insight.monthly_income || 0,
    periodExpense: insight.monthly_spending || 0,
    grossBudget: insight.budget_before_debt || 0,
    availableBudget: insight.available_budget_after_debt || 0,
    surplusAmount: insight.surplus_to_carry_next_period || 0,
    budgetCarryAmount: insight.budget_carry_to_next_period || 0,
    savingGoalContributionAmount: insight.saving_goal_contribution_amount || 0,
    debtAmount: insight.debt_to_carry_next_period || 0,
    debtRepaymentAmount: insight.debt_repayment_amount || 0,
    remainingDebtAmount: insight.debt_to_carry_next_period || 0,
    budgetAdjustmentAmount: 0,
    nextBudgetAmount: insight.next_budget_preview_amount || 0,
    warning: insight.debt_repayment_warning || '',
    isLimitedDebtRepayment: Boolean(insight.debt_repayment_limited)
  };
}
...
function buildFinancialRecommendations(context) {
  ...
  if (debtCarryover?.debtToCarryNextPeriod > 0) {
    if (debtCarryover.debtRepaymentLimited) {
      recommendations.push('Nợ lớn hơn ngân sách kỳ sau, chỉ nên trừ 25% nợ trước và giữ phần còn lại để trả dần.');
    } else {
      recommendations.push('Ưu tiên thanh toán khoản vượt ngân sách trước khi tăng chi tiêu ở kỳ tiếp theo.');
    }
  } else if (debtCarryover?.requiresSavingDecision) {
    recommendations.push('Bạn có tiền dư và có saving goal: hãy chọn giữ tiền cho budget kỳ sau, chia một phần vào saving goal, hoặc gửi hết vào saving goal.');
  } else if (debtCarryover?.budgetCarryToNextPeriod > 0) {
    recommendations.push('Không có saving goal đang hoạt động nên tiền dư cuối kỳ sẽ được cộng vào budget kỳ sau.');
  }
...
function buildFinancialInsightMessages(context) {
  ...
  if (debtCarryover?.debtToCarryNextPeriod > 0) {
    const limitedNote = debtCarryover.debtRepaymentLimited ? ' Hệ thống chỉ trừ 25% nợ vì nợ lớn hơn budget kỳ sau.' : '';
    insights.push(`Nếu kết thúc kỳ hiện tại, ${formatVnd(debtCarryover.debtToCarryNextPeriod)} sẽ bị carry-over sang kỳ sau.${limitedNote}`);
  } else if (debtCarryover?.surplusAmount > 0) {
    insights.push(`Kỳ này đang dư ${formatVnd(debtCarryover.surplusAmount)} để xử lý cho budget kỳ sau hoặc saving goal.`);
  }
...
app.post('/api/expenses', async (req, res) => {
  ...
  let debtStorage = getDebtStorageSource();
  if (type === 'expense' && Number(insightAfterSave.debt_to_carry_next_period || 0) > 0) {
    try {
      const persistedDebt = await upsertDebtCarryoverRecord(buildDebtSnapshotRecordFromInsight(user, insightAfterSave));
      debtStorage = persistedDebt.storage;
    } catch (error) {
      console.warn('Debt carry-over separate database sync failed:', error.message);
    }
  }
...
app.post('/api/debt-carryover/decision', async (req, res) => {
  const data = loadData();
  const user = findCurrentUser(req, data);
  if (!user) return res.status(401).json({ error: 'Unauthorized' });

  try {
    const result = applyDebtCarryoverDecision({
      data,
      user,
      getUserId,
      buildUserScopedData,
    ...
    const persistedDebt = await upsertDebtCarryoverRecord(result.decision, {
      user,
    ...
    res.json({
      ...result,
      debtDatabase: {
        storage: persistedDebt.storage,
        debt: persistedDebt.debt
      }
    });
  } catch (error) {
    console.error('Apply debt carry-over decision failed', error);
    res.status(error.status || 400).json({ error: error.message || 'Không thể áp dụng debt carry-over.' });
  }
});

app.get('/api/debt-carryover/history', async (req, res) => {
  const data = loadData();
  const user = findCurrentUser(req, data);
  const userId = getUserId(user);
  ...
  try {
    const [records, debts] = await Promise.all([
      getUserDebtCarryoverRecords(userId),
      getUserDebts(userId)
    ]);
    res.json({
    ...
    console.error('Load debt carry-over history failed', error);
    res.status(500).json({ error: 'Không thể tải lịch sử debt carry-over.' });
  }
});
